...
import { UserPlus, Users, Mail, Clock, Trash2, UserMinus, AlertTriangle, Download, Copy, CheckCircle } from "lucide-react";
...
export default function DashboardTeam() {
  ...
  const memberPagination = usePagination(members, { pageSize: 10 });
  ...
  const sendInvite = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success("Invitation sent! Share the invite link with your team member.");
    },
  ...
  const getRoleBadgeVariant = (role: string) => {
    switch (role) { case "admin": case "super_admin": return "default" as const; case "manager": return "secondary" as const; default: return "outline" as const; }
  };
  ...
  const handleExport = () => {
    exportToCsv("team-members", members, [
      { key: "full_name", label: "Name" }, { key: "role", label: "Role" }, { key: "created_at", label: "Joined" },
    ]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {permissions.canExportData && members.length > 0 && (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-display text-base flex items-center gap-2">
            <Users className="h-4 w-4" /> Members ({members.length} / {tenant?.max_users || 5})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="space-y-4">{[1, 2].map((i) => <div key={i} className="h-14 bg-muted rounded-lg animate-pulse" />)}</div>
          ) : members.length === 0 ? (
            <EmptyState icon={Users} title="No team members" description="Invite members to collaborate on projects and tasks." actionLabel={permissions.canInviteMembers ? "Invite Member" : undefined} onAction={permissions.canInviteMembers ? () => setInviteOpen(true) : undefined} />
          ) : (
            <>
                ...
                {memberPagination.items.map((member) => (
                  <div key={member.id} className="flex items-center justify-between py-3 border-b last:border-0">
                    ...
                    <div className="flex items-center gap-2">
                      {member.user_id === user?.id || !permissions.canManageTeam ? (
              ...
              <PaginationControls {...memberPagination} />
            </>
          )}
        </CardContent>
      </Card>
                ...
                <div key={inv.id} className="flex items-center justify-between py-3 border-b last:border-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => copyInviteLink(inv.token)} title="Copy invite link">
                      {copiedToken === inv.token ? <CheckCircle className="h-4 w-4 text-success" /> : <Copy className="h-4 w-4" />}
                    </Button>
